 ...
 * @link      http://pear.php.net/package/CreditCardValidation
 */
...
class CreditCardValidation
{
    ...
    {
        return CreditCardValidation::_luhn($number);
    }
     ...
     * @link http://www.speech.cs.cmu.edu/~sburke/pub/luhn_lib.html (Perl lib)
     */
    private static function _luhn($number)
    {
    ...
    public static function Number($creditCard, $cardType = null)
    {
        ...
        if (strlen($cc) != 15
            ...
            if (!CreditCardValidation::_luhn($cc)) {
                return false;
            }
        }

        if (is_string($cardType)) {
            return CreditCardValidation::Type($cc, $cardType);
        }
    ...
    public static function Type($creditCard, $cardType)
    {
        switch (strtoupper($cardType)) {
	        ...
	        case strtoupper(QCreditCardType::MasterCard):
	        case strtoupper(QCreditCardType::EuroCard):
	            $regex = '5[1-5][0-9]{14}';
	            break;
	        case 'VISA':
	        case strtoupper(QCreditCardType::Visa):
	            $regex = '4([0-9]{12}|[0-9]{15})';
	            break;
	        case 'AMEX':
	        case 'AMERICANEXPRESS':
	        case 'AMERICAN EXPRESS':
	        case strtoupper(QCreditCardType::Amex):
	            $regex = '3[47][0-9]{13}';
	            break;
	        case 'DINERS':
	        case 'DINERSCLUB':
	        ...
	        case 'CARTEBLANCHE':
	        case 'CARTE BLANCHE':
	        case strtoupper(QCreditCardType::CarteBlanche):
	        case strtoupper(QCreditCardType::Diners):
	            $regex = '3(0[0-5][0-9]{11}|[68][0-9]{12})';
	            break;
	        case 'DISCOVER':
	        case strtoupper(QCreditCardType::Discover):
	            $regex = '6011[0-9]{12}';
	            break;
	        case 'JCB':
	        case strtoupper(QCreditCardType::Jcb):
	            $regex = '(3[0-9]{15}|(2131|1800)[0-9]{11})';
	            break;
	        case 'ENROUTE':
	    	case strtoupper(QCreditCardType::Enroute):
	            $regex = '2(014|149)[0-9]{11}';
	            break;
	        case strtoupper(QCreditCardType::HiperCard):
	        	return true ;
	        default:
	            return false;
        }
