    ...
    public partial class Atualizacoes : System.Web.UI.UserControl
    {
        ...
        private void MontarPagina(MongoDB.Oid idUsuario)
        {
            ...
            if(contAtu == null || contAtu.Count == 0)
            {
                ...
                div.InnerHtml += string.Format("<time pubdate class='data'>{0}</time>", clsUtilidades.FormatarDataPublicacao(DateTime.Now));
            ...
            else
            {
                ...
                foreach (var atu in contAtu)
                {
                    ...
                    div.InnerHtml += string.Format("<time pubdate class='data'>{0}</time>", clsUtilidades.FormatarDataPublicacao(atu.CreatedTime));
                    div.InnerHtml += "<br /><br /><br />";
